define(["jquery", "ui-elements"], function($) {
	...
	var Lightbox = function(title, builderPath, saveFunction, width, exit_text, submit_text){
		if(exit_text == undefined)
			exit_text = "Exit without saving";
		if(submit_text == undefined)
			submit_text = 'Submit <img class="arrow-icon" src="'+builderPath+'UI_Elements/arrow.png"/>'
		if(width == undefined)
			width = 900;
	...
	Lightbox.prototype = {
		build: function(){
	        ...
	        addRoundCorners(this.$editPage, "3px");
	        processFont(this.$editPage, "AllerRegular", "#26202A", 15);
	        ...
	        this.$exitButton = $('<a href="#"></a>').css({
	            "width":"9px",
	            "height":"9px",
	            "background-image":"url("+this.builderPath+"UI_Elements/lightbox_close.png)",
	            "position":"relative",
	            "float":"right",
	            "left": "-10px",
	            "top":"6px"
	        ...
	        this.$loadingOptions = $('<div>Loading Options</div>').css({
	            ...
	            "display":"block",
	            "padding-left": "30px",
	            "background-image":"url("+this.builderPath+"UI_Elements/loading.gif)",
		...
		addContent: function(content){
			...
			this.$loadingOptions.css("display", "none");
